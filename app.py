...
# 1. Memuat data dari file pkl
with open('processed_berita_tempo.pkl', 'rb') as f:
    data_mentah = pickle.load(f)

# Mengubah data mentah menjadi Tabel (DataFrame)
df_berita = pd.DataFrame(data_mentah)

# --- INI PERBAIKANNYA ---
# Memaksa semua nama kolom (yang tadinya angka) menjadi teks (string)
df_berita.columns = df_berita.columns.astype(str)
# ------------------------
    ...
    for idx in top_indices:
        ...
        if skor > 0.0:  
            results.append({
                ...
                "isi": str(df_berita.iloc[idx][kolom_isi])[:250] + "...", 
